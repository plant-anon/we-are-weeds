...
/******/ 	var hotCurrentHash = "ec3adae231cdf28cfcf8"; // eslint-disable-line no-unused-vars
...
var WarpComponent = function (_Component) {
	_inherits(WarpComponent, _Component);

	function WarpComponent(type) {
		_classCallCheck(this, WarpComponent);

		var _this = _possibleConstructorReturn(this, (WarpComponent.__proto__ || Object.getPrototypeOf(WarpComponent)).call(this));
...
var WebComponent = function (_Component2) {
	_inherits(WebComponent, _Component2);

	function WebComponent(tag, attributes) {
		_classCallCheck(this, WebComponent);

		var _this2 = _possibleConstructorReturn(this, (WebComponent.__proto__ || Object.getPrototypeOf(WebComponent)).call(this));
		...
		if (attributes) {
			var attrs = Object.keys(attributes).map(function (x) {
				return x.toLowerCase();
			});

			if (attrs.includes('@text')) {
				_this2.text = attributes['@text'];
				delete attributes['@text'];
			}

			if (attrs.includes('@value')) {
				_this2.value = attributes['@value'];
	...
	_createClass(WebComponent, [{
		...
		value: function on(event, handler) {
			var _eventHandlers$push;

			(_eventHandlers$push = this.eventHandlers.push).call.apply(_eventHandlers$push, [this.eventHandlers].concat(Array.prototype.slice.call(arguments)));
		}
	}, {
		key: 'render',
		value: function render() {
			...
			for (var handle in this.eventHandlers) {
				...
				(_elem$on = elem.on).call.apply(_elem$on, [elem].concat(_toConsumableArray(handle)));
			}

			if (this.text) elem.text(this.text);

			if (this.value) elem.val(this.value);
...
exports.WarpComponent = WarpComponent;
...
var _warp = __webpack_require__(114);

var Warp = _interopRequireWildcard(_warp);

var _NavbarComponent = __webpack_require__(300);
...
var NavbarComponent = function (_Warp$WarpComponent) {
		_inherits(NavbarComponent, _Warp$WarpComponent);
		...
		_createClass(NavbarComponent, [{
				key: 'render',
				value: function render(element, attributes) {
						...
						//Mobile nav button
						$('<button type=button class="navbar-toggle collapsed" data-toggle=collapse data-target="#' + navid + '" aria-expanded=false aria-controls=navbar><span class=sr-only>Toggle navigation</span><span class="icon-bar"></span><span class=icon-bar></span><span class=icon-bar></span></button >').appendTo(container);
...
module.exports = function (components, render) {};
